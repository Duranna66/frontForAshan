...
function MatrixTest({ size }) {
    ...
    function dragStartHandlerList(e, x) {
        // console.log({ id: 0, prison_id: x.phoneNumber, animal_id: x.id, name:x.name})
        setCurrentItem([{id: 0, phoneNumber: x.phoneNumber, animal_id: x.id, name: x.name}]);
    }
    useEffect(() => {
    }, [currentItem]);
    ...
    function dropHandler(e, itemr) {
        ...
        else if(k1 === -1 && k2 !== -1) { //itemr is empty['', 'prison_id', '']
